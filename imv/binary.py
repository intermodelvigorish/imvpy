...
framework for evaluating feature importance in binary classification and regression models.
...
from .core import ll, get_w, calculate_imv
...
class BinaryIMV:
    ...
    from adding features to a model. It supports both classification and regression
    tasks with k-fold cross-validation or train-test split evaluation strategies.
    ...
    def __init__(self, data, outcome_variable, optional_explanatory_variables, model_creator, 
                 split_method, n_splits, prop_test, model_type, all_combinations_imv=None, random_seed=42):
        self.data = data
    ...
    def calculate_imv_score(self, model_basic, model_enhanced, X_basic, X_enhanced, y):
        ...
        Note:
            For classification, uses predict_proba()[:, 1] for positive class probability.
            For regression, uses predict() directly.
        """
    ...
    def plot_single_var_combinations_layered_violin_centralized_zero(self, ax=None, figsize=(6, 4)):
        ...
        if single_var_combinations:
            ...
            sns.violinplot(x='IMV Score', y='Variable', data=df, inner='quartile', 
                          palette='coolwarm', orient='h', ax=ax)
