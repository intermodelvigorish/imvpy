"""
SHAP-IMV Wine Quality Analysis
...
def load_and_prepare_wine_quality_data():
    """
    Load and prepare the Wine Quality dataset
    Features: 11 chemical variables
    Target: Dichotomized quality (0 for ≤5, 1 for >5)
    Missing values: None
    """
    data_file = os.path.join(DATA_DIR, 'wine_quality_processed.csv')
    ...
    try:
        ...
        # Prepare target variable - dichotomize quality
        # Quality is integer 1-10, we set: 0 for ≤5, 1 for >5
        if isinstance(y, pd.DataFrame):
            quality_col = y.columns[0]
            y_series = y[quality_col]
        else:
            y_series = y
        
        print(f"Original quality distribution:\n{y_series.value_counts().sort_index()}")
        
        # Dichotomize: 0 for quality ≤5, 1 for quality >5
        y_binary = (y_series > 5).astype(int)
        y_binary = pd.Series(y_binary, name='target')
        
        print(f"Dichotomized target distribution:\n{y_binary.value_counts()}")
        ...
        print(f"Features: {features}")
        print(f"Missing values after preprocessing: {df_final.isnull().sum().sum()}")
...
def save_model_results(model_name, shap_results, performance_results):
    ...
    filename = os.path.join(RESULTS_DIR, f'wine_quality_{model_name.replace(" ", "_").lower()}_results.pkl')
...
def plot_shap_imv_comparison(results, model_names_list):
    ...
    display_names = {
        'Logistic Regression': 'Wine Quality (Logistic Regression)',
        'XGBoost': 'Wine Quality (XGBoost)',
        'LightGBM': 'Wine Quality (LightGBM)'
    }
    ...
    plt.tight_layout()
    output_path = os.path.join(FIGURES_DIR, 'wine_quality_shap_imv_comparison.png')
...
def create_performance_table(performance_results, shap_results):
    ...
    """
    print("\n" + "="*100)
    print("PERFORMANCE COMPARISON TABLE (Wine Quality Dataset)")
    print("="*100)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SHAP-IMV Wine Quality Analysis')
    ...
    if args.clear_cache:
        import glob
        cache_files = glob.glob(os.path.join(RESULTS_DIR, 'wine_quality_*_results.pkl'))
    ...
    print("="*80)
    print("SHAP-IMV Wine Quality Analysis")
    if args.force:
        print("(FORCE MODE: Ignoring cached results)")
    print("="*80)
    
    # Load and prepare data
    print("\n1. Loading and preparing Wine Quality dataset...")
    df_wine, features = load_and_prepare_wine_quality_data()
    
    if df_wine is None or features is None:
    ...
    for model_name, model_creator in models.items():
        ...
        for i, seed in enumerate(seeds, 1):
            ...
            shap_values, evaluator = run_shap_imv_analysis(
                df_wine, model_name, model_creator, seed, features
            )
            model_shap_results[seed] = shap_values
            
            # Calculate performance metrics
            acc, prec = calculate_performance_metrics(
                model_creator, df_wine, features, seed
            )
