"""
SHAP-IMV Breast Cancer Analysis
...
def load_and_prepare_breast_cancer_data():
    """
    Load and prepare the Breast Cancer Wisconsin (Original) dataset
    Features: 9 integer variables measuring tumor characteristics
    Target: Binary (benign=0, malignant=1)
    Missing values: < 1%, use mean imputation
    """
    data_file = os.path.join(DATA_DIR, 'breast_cancer_processed.csv')
    ...
    try:
        ...
        print("Fetching Breast Cancer Wisconsin (Original) dataset...")
        # Breast Cancer Wisconsin (Original) dataset ID: 15
        ...
        # Handle missing values - mean imputation for numerical features
        ...
        # Class: 2 for benign, 4 for malignant -> convert to 0 and 1
        y_series = y.iloc[:, 0] if isinstance(y, pd.DataFrame) else y
        y_binary = (y_series == 4).astype(int)  # 1 for malignant, 0 for benign
        y_binary = pd.Series(y_binary, name='target')
        
        # Combine features and target
        df_final = pd.concat([X, y_binary], axis=1)
...
def save_model_results(model_name, shap_results, performance_results):
    ...
    filename = os.path.join(RESULTS_DIR, f'breast_cancer_{model_name.replace(" ", "_").lower()}_results.pkl')
...
def plot_shap_imv_comparison(results, model_names_list):
    """
    Create bar plots comparing SHAP-IMV values across models
    ...
    display_names = {
        'Logistic Regression': 'Breast Cancer (Logistic Regression)',
        'XGBoost': 'Breast Cancer (XGBoost)',
        'LightGBM': 'Breast Cancer (LightGBM)'
    }
    ...
    plt.tight_layout()
    output_path = os.path.join(FIGURES_DIR, 'breast_cancer_shap_imv_comparison.png')
...
def create_performance_table(performance_results, shap_results):
    ...
    """
    print("\n" + "="*100)
    print("PERFORMANCE COMPARISON TABLE (Breast Cancer Dataset)")
    print("="*100)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SHAP-IMV Breast Cancer Analysis')
    ...
    if args.clear_cache:
        import glob
        cache_files = glob.glob(os.path.join(RESULTS_DIR, 'breast_cancer_*_results.pkl'))
    ...
    print("="*80)
    print("SHAP-IMV Breast Cancer Analysis")
    if args.force:
        print("(FORCE MODE: Ignoring cached results)")
    print("="*80)
    
    # Load and prepare data
    print("\n1. Loading and preparing Breast Cancer dataset...")
    df_cancer, features = load_and_prepare_breast_cancer_data()
    
    if df_cancer is None or features is None:
    ...
    for model_name, model_creator in models.items():
        ...
        for i, seed in enumerate(seeds, 1):
            ...
            shap_values, evaluator = run_shap_imv_analysis(
                df_cancer, model_name, model_creator, seed, features
            )
            model_shap_results[seed] = shap_values
            
            # Calculate performance metrics
            acc, prec = calculate_performance_metrics(
                model_creator, df_cancer, features, seed
            )
