"""
Multi-IMV Car Evaluation Analysis
...
def load_and_prepare_car_evaluation_data():
    """
    Load and prepare the Car Evaluation dataset
    4 classes: unacceptable, acceptable, good, very good
    Features: 6 categorical variables about car characteristics
    """
    data_file = os.path.join(DATA_DIR, 'car_evaluation_processed.csv')
    ...
    try:
        ...
        # Map: unacc->0, acc->1, good->2, vgood->3
        class_mapping = {
            'unacc': 0,
            'acc': 1,
            'good': 2,
...
def save_model_results(model_name, imv_results, performance_results):
    ...
    filename = os.path.join(RESULTS_DIR, f'car_evaluation_{model_name.replace(" ", "_").lower()}_results.pkl')
...
def plot_imv_confusion_matrix(imv_matrix, model_name, class_names):
    ...
    ax.set_title(f'Car Evaluation ({model_name})', fontsize=14, fontweight='bold')
    ax.set_xlabel('Predicted Class', fontsize=12)
    ax.set_ylabel('True Class', fontsize=12)
    
    plt.tight_layout()
    output_path = os.path.join(FIGURES_DIR, f'car_evaluation_multi_imv_{model_name.replace(" ", "_").lower()}.png')
...
def create_performance_table(performance_results, imv_results, class_names):
    ...
    print("\n" + "="*120)
    print("PERFORMANCE COMPARISON TABLE (Car Evaluation Dataset)")
    print("="*120)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Multi-IMV Car Evaluation Analysis')
    ...
    if args.clear_cache:
        import glob
        cache_files = glob.glob(os.path.join(RESULTS_DIR, 'car_evaluation_*_results.pkl'))
    ...
    print("="*80)
    print("Multi-IMV Car Evaluation Analysis")
    if args.force:
        print("(FORCE MODE: Ignoring cached results)")
    print("="*80)
    
    print("\n1. Loading and preparing Car Evaluation dataset...")
    df_car, features, classes = load_and_prepare_car_evaluation_data()
    
    if df_car is None or features is None:
